...
def build_queries(entity: Entity, deep: bool = True) -> list[str]:
    ...
    if t is EntityType.NAME:
        ...
        # (***.456.789-**). Sem essas consultas o CPF quase nunca é achado.
        if miolo:
            q += [
                f'"{miolo}" CPF',
                f'"{miolo}" (edital OR nomeação OR portaria OR contrato OR processo)',
            ]
        if deep:
            q += [
                f'"{v}" (filetype:pdf OR filetype:xls OR filetype:xlsx OR filetype:csv)',
